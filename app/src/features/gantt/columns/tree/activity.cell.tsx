...
export const ActivityTreeCell = observer((props: GridRenderCellParams<Activity | Assignment>) => {
    ...
    const handleTreeToggle = () => {
        if (rowNode.type !== "group" || props.row.kind === "Assignment") return;
    ...
    const isActionVisible = computed(() => {
        return props.row.kind === "Assignment" ? false : props.row.children.length > 0
    });
