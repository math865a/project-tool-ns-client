...
import { MenuItem, ListItemIcon, Box, ListItemText, ListItemSecondaryAction } from "@mui/material";
...
export const ContextMenuColorPickerItem = observer(({ Activity }: { Activity: Activity }) => {
    const {Gantt: {Table: {ContextMenu: C}}} = useWorkpackage()
    const M = C.ColorMenu;
    ...
    return (
        <>
            <MenuItem sx={{ py: 1 }} ref={ref} onClick={() => M.handleOpen()}>
                <ListItemIcon>
                    <Box ml={-0.25}>
                        <ColorPickerOption
                            color={Activity.fill}
