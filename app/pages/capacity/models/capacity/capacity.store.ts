...
import { Bound, CapacityBatchFilter, CapacityJson } from '@math865a/project-tool.types';
...
export class CapacityStore extends Model({
    ...
    resolve(json: CapacityJson) {
        ...
        if (Row) {
            ...
            Row = new Capacity({
                ...
                Interval: new CapacityInterval({ts: normalize(json.interval.ts).toMillis(), tf: normalize(json.interval.tf).toMillis()}),
                Stats: new CapacityStats(json.stats),
            });
    ...
    removeRowCapacities(Row: ResourceRow){
        this.Capacities = _.filter(this.Capacities, d => !Row.Capacities.includes(d))
    }
}
